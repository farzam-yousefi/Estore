...
// const categories=await selectCollectionDos<CategoryClient>("categories");
...
export default function AdminCategoriesPage() {
  // const categories=await selectCollectionDos<CategoryClient>("categories");
  // const[categories_,setCategories_]=useState<CategoryClient[]>(categories ?? [])
   
  const [open, setOpen] = useState(false);
  const [activeCategory, setActiveCategory] = useState<CategoryClient | null>(null);
          ...
          <thead className="border-b bg-muted">
           ...
           { categories.map((cat) => (
              <tr key={cat._id} className="border-b last:border-none">
                ...
                <td className="px-3 py-2 items-center">{(cat.baseProperties) ? (cat.baseProperties).length : 0}</td>
  ...
  ">
    ...
    <div className="p-4 space-y-4 overflow-y-auto h-[calc(100vh-6rem)]">
      <div className="space-y-1">
              ...
              <Input defaultValue={activeCategory?.name ?? ""} 
            ...
            <div className="space-y-1">
              ...
              <Input defaultValue={activeCategory?.slug ?? ""}
